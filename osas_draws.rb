...
def make_graphic_for_commit(commit, shell_out = true) 
  shape=[] << ('make new shape at end of graphics with properties ' + 
               '{name:\"RoundRect\", textSize:{0.8, 1.0}, ' + 
               'fill color:{0.718475, 1.0, 0.755425}, ' +
               'textPosition:{0.1, 0.0}, ' + 
               'origin:{206.0, 270.0}, ' +
               'size:{121.0, 79.0}, ' +
               'draws shadow:false, ' + 
               'text: {text: \"' + commit.id[0,8] + '\", text: \"' + commit.id[0,8] + '\", alignment: center}}'
              )
...
def make_graphic_for_tag(tag, shell_out = true)
  tag=[] << ('make new shape at end of graphics with properties ' + 
             '{fill color: {0.760290, 0.952503, 1.000000}, ' + 
             'origin: {60.000000, 84.000000}, ' + 
             'size: {140.000000, 66.000000}, ' + 
             'text: {text: \"' + tag.name + '\", text: \"' + tag.name + '\", alignment: center}, ' + 
             'draws shadow: false}'
            )
...
def make_graphic_for_head(head, shell_out = true)
  head=[] << ('make new shape at end of graphics with properties {name: \"Circle\", ' + 
              'textSize: {0.800000, 0.700000}, ' + 
              'fill color: {0.960518, 0.679012, 1.000000}, ' + 
              'textPosition: {0.100000, 0.150000}, ' + 
              'origin: {160.000000, 120.112358}, ' + 
              'size: {118.000000, 39.775280}, ' + 
              'text: {text: \"' + head.name + '\", text: \"' + head.name + '\", alignment: center}, ' + 
              'draws shadow: false}'
             )
...
def make_line_between_graphics(commit, parent, shell_out = true)
  linescript=[] << ('make new line at end of graphics with properties ' + 
                    '{point list:{{255.7701, 269.5171}, {229.7299, 172.4829}}, ' + 
                    'head type:\"FilledArrow\", ' + 
                    'line type:curved}' )
